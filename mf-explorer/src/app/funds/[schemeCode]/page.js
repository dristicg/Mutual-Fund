...
export default function FundDetailPage() {
  ...
  const [rollingStartDate, setRollingStartDate] = useState("2020-01-01");
  const [rollingEndDate, setRollingEndDate] = useState(new Date().toISOString().split('T')[0]);
  ...
  const handleRollingReturnsCalculate = async () => {
    ...
    try {
      const res = await fetch(`/api/scheme/${schemeCode}/rolling-returns?period=${rollingPeriod}&from=${rollingStartDate}&to=${rollingEndDate}`);
      const result = await res.json();
  ...
  return (
    <Container maxWidth="xl" sx={{ mt: 4, mb: 4 }}>
          ...
          {/* NAV Chart & Returns Table - Side by Side with Flexbox */}
          <Box sx={{ display: 'flex', gap: 2, mb: 4, flexDirection: { xs: 'column', md: 'row' } }}>
            {/* NAV Chart - Takes most of the width */}
            <Box sx={{ flex: '0 0 75%', minWidth: 0 }}>
              <Card elevation={3} sx={{ bgcolor: 'rgba(30, 41, 59, 0.8)', border: '1px solid rgba(99, 102, 241, 0.3)', height: '100%' }}>
                <CardContent sx={{ p: 3 }}>
                  <Box display="flex" justifyContent="space-between" alignItems="center" mb={2} flexWrap="wrap" gap={1}>
                    <Typography variant="h6" fontWeight={600} color="#fff">NAV History</Typography>
                    <Box>
                      <Button size="small" onClick={() => setNavPeriod('6m')} variant={navPeriod === '6m' ? 'contained' : 'outlined'} sx={{ mr: 1, mb: { xs: 1, sm: 0 } }}>6M</Button>
                      <Button size="small" onClick={() => setNavPeriod('1y')} variant={navPeriod === '1y' ? 'contained' : 'outlined'} sx={{ mr: 1, mb: { xs: 1, sm: 0 } }}>1Y</Button>
                      <Button size="small" onClick={() => setNavPeriod('3y')} variant={navPeriod === '3y' ? 'contained' : 'outlined'} sx={{ mr: 1, mb: { xs: 1, sm: 0 } }}>3Y</Button>
                      <Button size="small" onClick={() => setNavPeriod('5y')} variant={navPeriod === '5y' ? 'contained' : 'outlined'} sx={{ mr: 1, mb: { xs: 1, sm: 0 } }}>5Y</Button>
                      <Button size="small" onClick={() => setNavPeriod('all')} variant={navPeriod === 'all' ? 'contained' : 'outlined'} sx={{ mb: { xs: 1, sm: 0 } }}>All</Button>
                    </Box>
                  </Box>
                  <Box sx={{ height: 500, mt: 2, width: '100%' }}>
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={navData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
            ...
            {/* Returns Table - Takes remaining width */}
            <Box sx={{ flex: '0 0 25%', minWidth: 250 }}>
          ...
          {calculatorTab === 3 && (
            <Grid container spacing={3}>
              <Grid item xs={12} md={4}>
                <Paper component="form" onSubmit={handleStepUpSipCalculate} elevation={3} sx={{ p: 3, position: 'sticky', top: 80, bgcolor: 'rgba(30, 41, 59, 0.8)', border: '1px solid rgba(99, 102, 241, 0.3)' }}>
                  <Typography variant="h6" mb={3} fontWeight={600} color="#fff">Step-Up SIP Calculator</Typography>
                  <Grid container spacing={2}>
                    <Grid item xs={12}>
              ...
              <Grid item xs={12} md={8}>
                {stepUpSipResult && !stepUpSipResult.error && (
                  <Card elevation={3} sx={{ bgcolor: 'rgba(30, 41, 59, 0.8)', border: '1px solid rgba(99, 102, 241, 0.3)' }}>
                    <CardContent sx={{ p: 3 }}>
                      <Typography variant="h5" gutterBottom fontWeight={600} color="#fff">Step-Up SIP Results</Typography>
                      <Grid container spacing={2} sx={{ mb: 3 }}>
                        <Grid item xs={6} sm={3}>
                          <Box sx={{ p: 2, borderRadius: 1, bgcolor: 'rgba(96, 165, 250, 0.1)' }}>
                            <Typography variant="caption" sx={{ color: '#94a3b8' }}>Invested</Typography>
                            <Typography variant="h6" fontWeight={600} color="#fff">₹{formatNumber(stepUpSipResult.totalInvested)}</Typography>
                          </Box>
                        </Grid>
                        <Grid item xs={6} sm={3}>
                          <Box sx={{ p: 2, borderRadius: 1, bgcolor: 'rgba(96, 165, 250, 0.1)' }}>
                            <Typography variant="caption" sx={{ color: '#94a3b8' }}>Current Value</Typography>
                            <Typography variant="h6" fontWeight={600} color="#fff">₹{formatNumber(stepUpSipResult.currentValue)}</Typography>
                          </Box>
                        </Grid>
                        <Grid item xs={6} sm={3}>
                          <Box sx={{ p: 2, borderRadius: 1, bgcolor: 'rgba(34, 197, 94, 0.1)' }}>
                            <Typography variant="caption" sx={{ color: '#94a3b8' }}>Gain</Typography>
                            <Typography variant="h6" sx={{ color: '#22c55e' }} fontWeight={600}>₹{formatNumber(stepUpSipResult.currentValue - stepUpSipResult.totalInvested)}</Typography>
                          </Box>
                        </Grid>
                        <Grid item xs={6} sm={3}>
                          <Box sx={{ p: 2, borderRadius: 1, bgcolor: 'rgba(167, 139, 250, 0.1)' }}>
                            <Typography variant="caption" sx={{ color: '#94a3b8' }}>CAGR</Typography>
                            <Typography variant="h6" fontWeight={600} color="#fff">{formatNumber(stepUpSipResult.annualizedReturn)}%</Typography>
                          </Box>
                        </Grid>
                      </Grid>
                      <Divider sx={{ my: 3 }} />
                      <Typography variant="h6" gutterBottom fontWeight={600} color="#fff">Growth Chart</Typography>
                      <Box sx={{ height: 300, mt: 2 }}>
                        <ResponsiveContainer width="100%" height="100%">
                          <LineChart data={stepUpSipResult.growth} margin={{ top: 5, right: 20, left: 20, bottom: 5 }}>
                            <CartesianGrid strokeDasharray="3 3" stroke="#334155" />
                            <XAxis dataKey="date" stroke="#94a3b8" />
                            <YAxis width={80} tickFormatter={(value) => `₹${(value/1000)}k`} stroke="#94a3b8" />
                            <Tooltip formatter={(value) => `₹${formatNumber(value)}`} contentStyle={{ backgroundColor: '#1e293b', border: '1px solid #334155' }} />
                ...
                {!stepUpSipResult && <Alert severity="info" sx={{ bgcolor: 'rgba(96, 165, 250, 0.1)' }}>Enter Step-Up SIP details and click "Calculate Returns"</Alert>}
              </Grid>
            </Grid>
          )}
          ...
          {calculatorTab === 4 && (
            <Grid container spacing={3}>
              <Grid item xs={12} md={4}>
                <Paper component="form" onSubmit={handleStepUpSwpCalculate} elevation={3} sx={{ p: 3, position: 'sticky', top: 80, bgcolor: 'rgba(30, 41, 59, 0.8)', border: '1px solid rgba(99, 102, 241, 0.3)' }}>
                  <Typography variant="h6" mb={3} fontWeight={600} color="#fff">Step-Up SWP Calculator</Typography>
                  <Grid container spacing={2}>
                    <Grid item xs={12}>
              ...
              <Grid item xs={12} md={8}>
                {stepUpSwpResult && !stepUpSwpResult.error && (
                  <Card elevation={3} sx={{ bgcolor: 'rgba(30, 41, 59, 0.8)', border: '1px solid rgba(99, 102, 241, 0.3)' }}>
                    <CardContent sx={{ p: 3 }}>
                      <Typography variant="h5" gutterBottom fontWeight={600} color="#fff">Step-Up SWP Results</Typography>
                      {stepUpSwpResult.exhausted && (
                        <Alert severity="warning" sx={{ mb: 2 }}>⚠️ Investment exhausted before end date!</Alert>
                      )}
                      <Grid container spacing={2} sx={{ mb: 3 }}>
                        <Grid item xs={6} sm={3}>
                          <Box sx={{ p: 2, borderRadius: 1, bgcolor: 'rgba(96, 165, 250, 0.1)' }}>
                            <Typography variant="caption" sx={{ color: '#94a3b8' }}>Initial</Typography>
                            <Typography variant="h6" fontWeight={600} color="#fff">₹{formatNumber(stepUpSwpResult.initialInvestment)}</Typography>
                          </Box>
                        </Grid>
                        <Grid item xs={6} sm={3}>
                          <Box sx={{ p: 2, borderRadius: 1, bgcolor: 'rgba(239, 68, 68, 0.1)' }}>
                            <Typography variant="caption" sx={{ color: '#94a3b8' }}>Withdrawn</Typography>
                            <Typography variant="h6" sx={{ color: '#ef4444' }} fontWeight={600}>₹{formatNumber(stepUpSwpResult.totalWithdrawn)}</Typography>
                          </Box>
                        </Grid>
                        <Grid item xs={6} sm={3}>
                          <Box sx={{ p: 2, borderRadius: 1, bgcolor: 'rgba(34, 197, 94, 0.1)' }}>
                            <Typography variant="caption" sx={{ color: '#94a3b8' }}>Remaining</Typography>
                            <Typography variant="h6" sx={{ color: '#22c55e' }} fontWeight={600}>₹{formatNumber(stepUpSwpResult.remainingValue)}</Typography>
                          </Box>
                        </Grid>
                        <Grid item xs={6} sm={3}>
                          <Box sx={{ p: 2, borderRadius: 1, bgcolor: 'rgba(167, 139, 250, 0.1)' }}>
                            <Typography variant="caption" sx={{ color: '#94a3b8' }}>Total Value</Typography>
                            <Typography variant="h6" sx={{ color: '#a855f7' }} fontWeight={600}>₹{formatNumber(stepUpSwpResult.totalValue)}</Typography>
                          </Box>
                        </Grid>
                      </Grid>
                      <Divider sx={{ my: 3 }} />
                      <Typography variant="h6" gutterBottom fontWeight={600} color="#fff">Withdrawal Chart</Typography>
                      <Box sx={{ height: 300, mt: 2 }}>
                        <ResponsiveContainer width="100%" height="100%">
                          <AreaChart data={stepUpSwpResult.growth} margin={{ top: 5, right: 20, left: 20, bottom: 5 }}>
                            <defs>
                            ...
                            <Tooltip formatter={(value) => `₹${formatNumber(value)}`} contentStyle={{ backgroundColor: '#1e293b', border: '1px solid #334155' }} />
                            <Legend />
                            <Area type="monotone" dataKey="remainingValue" name="Remaining Value" stroke="#22c55e" strokeWidth={2} fillOpacity={1} fill="url(#colorStepUpSwpRemaining)" />
                          </AreaChart>
                        </ResponsiveContainer>
                      </Box>
                    </CardContent>
                ...
                {!stepUpSwpResult && <Alert severity="info" sx={{ bgcolor: 'rgba(96, 165, 250, 0.1)' }}>Enter Step-Up SWP details and click "Calculate Withdrawals"</Alert>}
              </Grid>
            </Grid>
          )}
          ...
          {calculatorTab === 5 && (
            <Grid container spacing={3}>
              <Grid item xs={12} md={4}>
                <Paper elevation={3} sx={{ p: 3, position: 'sticky', top: 80, bgcolor: 'rgba(30, 41, 59, 0.8)', border: '1px solid rgba(99, 102, 241, 0.3)' }}>
                  <Typography variant="h6" mb={3} fontWeight={600} color="#fff">Rolling Returns</Typography>
                  <Grid container spacing={2}>
                    <Grid item xs={12}>
                      <TextField label="Rolling Period" value={rollingPeriod} onChange={e => setRollingPeriod(e.target.value)} fullWidth select required>
                        <MenuItem value="1y">1 Year Rolling</MenuItem>
                        <MenuItem value="3y">3 Years Rolling</MenuItem>
                        <MenuItem value="5y">5 Years Rolling</MenuItem>
                      </TextField>
                    </Grid>
                    <Grid item xs={12}>
                      <TextField label="Start Date" type="date" value={rollingStartDate} onChange={e => setRollingStartDate(e.target.value)} fullWidth InputLabelProps={{ shrink: true }} required />
                    </Grid>
                    <Grid item xs={12}>
                      <TextField label="End Date" type="date" value={rollingEndDate} onChange={e => setRollingEndDate(e.target.value)} fullWidth InputLabelProps={{ shrink: true }} required />
                    </Grid>
                    <Grid item xs={12}>
              ...
              <Grid item xs={12} md={8}>
                {rollingReturns && !rollingReturns.error && (
                  <Card elevation={3} sx={{ bgcolor: 'rgba(30, 41, 59, 0.8)', border: '1px solid rgba(99, 102, 241, 0.3)' }}>
                    <CardContent sx={{ p: 3 }}>
                      <Typography variant="h5" gutterBottom fontWeight={600} color="#fff">Rolling Returns Analysis</Typography>
                      <Grid container spacing={2} sx={{ mb: 3 }}>
                        <Grid item xs={6} sm={3}>
                          <Box sx={{ p: 2, borderRadius: 1, bgcolor: 'rgba(34, 197, 94, 0.1)' }}>
                            <Typography variant="caption" sx={{ color: '#94a3b8' }}>Average</Typography>
                            <Typography variant="h6" fontWeight={600} sx={{ color: '#22c55e' }}>{formatNumber(rollingReturns.average)}%</Typography>
                          </Box>
                        </Grid>
                        <Grid item xs={6} sm={3}>
                          <Box sx={{ p: 2, borderRadius: 1, bgcolor: 'rgba(96, 165, 250, 0.1)' }}>
                            <Typography variant="caption" sx={{ color: '#94a3b8' }}>Max</Typography>
                            <Typography variant="h6" fontWeight={600} color="#fff">{formatNumber(rollingReturns.max)}%</Typography>
                          </Box>
                        </Grid>
                        <Grid item xs={6} sm={3}>
                          <Box sx={{ p: 2, borderRadius: 1, bgcolor: 'rgba(239, 68, 68, 0.1)' }}>
                            <Typography variant="caption" sx={{ color: '#94a3b8' }}>Min</Typography>
                            <Typography variant="h6" fontWeight={600} sx={{ color: '#ef4444' }}>{formatNumber(rollingReturns.min)}%</Typography>
                          </Box>
                        </Grid>
                        <Grid item xs={6} sm={3}>
                          <Box sx={{ p: 2, borderRadius: 1, bgcolor: 'rgba(251, 191, 36, 0.1)' }}>
                            <Typography variant="caption" sx={{ color: '#94a3b8' }}>Std Dev</Typography>
                            <Typography variant="h6" fontWeight={600} sx={{ color: '#fbbf24' }}>{formatNumber(rollingReturns.stdDev)}%</Typography>
                          </Box>
                        </Grid>
                      </Grid>
                      <Divider sx={{ my: 3 }} />
                      <Typography variant="h6" gutterBottom fontWeight={600} color="#fff">Rolling Returns Chart</Typography>
                      <Box sx={{ height: 300, mt: 2 }}>
                        <ResponsiveContainer width="100%" height="100%">
                          <LineChart data={rollingReturns.data} margin={{ top: 5, right: 20, left: 20, bottom: 5 }}>
                            <CartesianGrid strokeDasharray="3 3" stroke="#334155" />
                            <XAxis dataKey="date" stroke="#94a3b8" tick={{ fontSize: 12 }} />
                            <YAxis stroke="#94a3b8" tickFormatter={(v) => `${v.toFixed(0)}%`} />
                            <Tooltip formatter={(v) => `${formatNumber(v)}%`} contentStyle={{ backgroundColor: '#1e293b', border: '1px solid #334155', borderRadius: 8 }} />
                            <Legend />
                            <Line type="monotone" dataKey="return" name="Rolling Return" stroke="#60a5fa" strokeWidth={2} dot={false} />
                          </LineChart>
                        </ResponsiveContainer>
                      </Box>
                    </CardContent>
                ...
                {!rollingReturns && <Alert severity="info" sx={{ bgcolor: 'rgba(96, 165, 250, 0.1)' }}>Select a period and click "Calculate Rolling Returns"</Alert>}
              </Grid>
            </Grid>
          )}
